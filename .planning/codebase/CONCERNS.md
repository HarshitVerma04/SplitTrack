...
- App state uses only the first group and first expense found for dashboard detail sections.
  - `splittrack-backend/src/main/java/com/splittrack/backend/app/service/AppStateService.java`.
...
- Expense split type is stored as `SplitType.EXACT` and only supports explicit amounts.
  - `splittrack-backend/src/main/java/com/splittrack/backend/app/service/AppCommandService.java`.
  - UI exposes equal and percentage splits in `splittrack-frontend-v1/src/pages/AddExpensePage.tsx`.

## Notifications are derived only from settlements
- No notifications table or event log exists.
  - `splittrack-backend/src/main/resources/db/migration/V2__init_expense_domain.sql`.
...
- `splittrack-ui/` and `splittrack-web/` appear unused by the primary app.
