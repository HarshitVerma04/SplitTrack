...
function AuthLayout({ title, subtitle, cta, ctaHref }: { title: string; subtitle: string; cta: string; ctaHref: string }) {
  return (
    <div className="grid min-h-screen place-items-center bg-[#f8f9fa] px-4 dark:bg-[#191c1d]">
      ...
      >
        ...
        <div className="mt-6 space-y-3">
          <input className="w-full rounded-lg bg-[#edeeef] p-3 text-sm outline-none ring-[#4c1b87]/30 focus:ring-2 dark:bg-[#1e1e1e]" placeholder="Email" />
          <input className="w-full rounded-lg bg-[#edeeef] p-3 text-sm outline-none ring-[#4c1b87]/30 focus:ring-2 dark:bg-[#1e1e1e]" placeholder="Password" type="password" />
          <button className="w-full rounded-lg bg-gradient-to-br from-[#4c1b87] to-[#6437a0] py-3 text-sm font-bold text-white transition hover:scale-[1.01]">
            Continue
          </button>
        </div>
...
export function LoginPage() {
  return <AuthLayout title="Welcome Back" subtitle="Login to continue" cta="Create account" ctaHref="/signup" />
}

export function SignupPage() {
  return <AuthLayout title="Create Account" subtitle="Start splitting expenses" cta="Already have an account" ctaHref="/login" />
}
